...
static size_t exponent_to_string(char *buf, int32_t exponent) {
    ...
    unsigned int i = pos;
    do {
        unsigned int digit = (uint32_t)exponent % 10;
...
static char *double_to_string(char *buf, size_t len, double d, unsigned int flag) {
    size_t pos = 0;
...
#define OUTREV(c) do { if (&buf[pos] == buf) goto done; else buf[--pos] = (c); } while (0)
            ...
            unsigned int i = decimal_spot + 6 + 1;
            while (frac != 0) {
                unsigned int digit = frac % 10;
...
static char *double_to_hexstring(char *buf, size_t len, double d, unsigned int flag) {
    size_t pos = 0;
    ...
    if (exponent == 0x7ff) {
        ...
        for (int i = 52 - 4; i >= 0; i -= 4) {
            unsigned int digit = (fraction >> i) & 0xf;
        ...
        switch (c) {
            ...
            case 'd':
                n = (flags & LONGLONGFLAG) ? va_arg(ap, long long) :
                    ...
                    (flags & INTMAXFLAG) ? va_arg(ap, long long) :
            ...
            case 'u':
                n = (flags & LONGLONGFLAG) ? va_arg(ap, unsigned long long) :
                    ...
                    (flags & SIZETFLAG) ? va_arg(ap, size_t) :
                    (flags & INTMAXFLAG) ? va_arg(ap, unsigned long long) :
