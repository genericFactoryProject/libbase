...
{
	return NULL; //kmem_cache_alloc(maple_node_cache, gfp);
}

static inline int mt_alloc_bulk(gfp_t gfp, size_t size, void **nodes)
{
	return 0; // kmem_cache_alloc_bulk(maple_node_cache, gfp, size, nodes);
}
 ...
 * @count: The number of nodes needed
 *
 * Note: Uses 0 | 0 for gfp flags.
 */
static void mas_node_count(struct ma_state *mas, int count)
{
	return mas_node_count_gfp(mas, count, 0 | 0);
}
...
int mas_expected_entries(struct ma_state *mas, unsigned long nr_entries)
{
	...
	mas_node_count_gfp(mas, nr_nodes + 3, 0);
	...
	__must_hold(mas->tree->ma_lock)
{
	...
	if (!mt_external_lock(mas->tree)) { // gfpflags_allow_blocking(gfp) &&
